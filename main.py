...
async def survey_write_page(survey_id: int, session_token: str = Cookie(default=None)):
    ...
    for q in question_rows:
        ...
        questions_data.append({
            ...
            "link_url": q["link_url"] or "",
            "link_label": q["link_label"] or "",
...
async def survey_submit(survey_id: int, request: Request, session_token: str = Cookie(default=None)):
    ...
    for qid, a_list in answers_by_qid.items():
        ...
        q_points = float(q["score_points"]) if q["score_points"] is not None else 0
        max_score += q_points
        if is_correct:
...
async def master_survey_questions_page(survey_id: int, session_token: str = Cookie(default=None)):
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px;">
        <div style="flex:1;">
          <label style="font-size:12px;color:#888;">참고 링크 URL (선택)</label>
          <input type="text" id="qLinkUrl" placeholder="https://...">
        </div>
        <div style="flex:1;">
          <label style="font-size:12px;color:#888;">링크 표시 텍스트 (선택)</label>
          <input type="text" id="qLinkLabel" placeholder="예: 매뉴얼 보기">
        </div>
      </div>
    ...
    <script>
      ...
      async function saveQuestion() {{
        ...
        const linkUrl = document.getElementById('qLinkUrl').value.trim();
        const linkLabel = document.getElementById('qLinkLabel').value.trim();
        ...
        const payload = {{
          ...
          image_url: imageUrl, link_url: linkUrl, link_label: linkLabel,
...
async def master_branch_manage_page(session_token: str = Cookie(default=None)):
    ...
    <div class="card">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;max-width:400px;">
        ...
        <label style="font-size:12px;color:#888;">소속 팀 (선택, 지점만 해당)</label>
        <select id="newBranchTeam">
    ...
    <div class="card">
      <table>
        <thead><tr><th>지점명</th><th>지점코드</th><th>로그인ID</th><th>비밀번호</th><th>구분</th><th>팀</th><th></th></tr></thead>
        <tbody>{rows_html}</tbody>
      </table>
    </div>
    ...
    <div id="editBranchModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;">
      <div style="background:#fff;border-radius:12px;padding:24px;max-width:400px;width:90%;">
        ...
        <label style="font-size:13px;color:#555;">소속 팀</label>
        <select id="editBranchTeam" style="width:100%;padding:10px;border:1px solid #ccc;border-radius:6px;margin-bottom:16px;box-sizing:border-box;">
          <option value="">미배정</option>
          <option value="1팀">1팀</option>
          <option value="2팀">2팀</option>
          <option value="3팀">3팀</option>
    ...
    <script>
      ...
      function editBranch(branchCode, branchName, loginId, password, branchType, branchTeam) {{
        ...
        document.getElementById('editBranchTeam').value = branchTeam || '';
...
async def purchase_order_product_settings_page(
    ...
    if not rows:
        ...
        for r in rows:
            ...
            <tr>
              ...
              <td class="pm-col-price"><input type="number" class="pm-price" data-id="{r['id']}" value="{r['purchase_price'] or 0}"></td>
              <td class="pm-col-supplier">{r['supplier'] or ''}</td>
              <td class="pm-col-tax">{r['tax_setting'] or ''}</td>
    ...
    <div class="card">
      ...
      <table class="pm-table">
        <thead><tr>
          ...
          <th class="pm-col-price" style="cursor:pointer;" onclick="sortPm('purchase_price')">구매금액 {'▲' if sort_by=='purchase_price' and sort_dir=='asc' else ('▼' if sort_by=='purchase_price' else '')}</th>
          <th class="pm-col-supplier">거래처</th>
          <th class="pm-col-tax">과세설정</th>
    ...
    <div id="pmColSettingsModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;">
      <div style="background:#fff;border-radius:12px;padding:24px;max-width:360px;width:90%;">
        ...
        <div id="pmColCheckboxList" style="display:flex;flex-direction:column;gap:8px;font-size:14px;"></div>
        <style>#pmColCheckboxList input[type="checkbox"] {{ width: auto !important; }}</style>
        <div style="display:flex;gap:8px;margin-top:16px;">
          <button class="btn" style="flex:1;background:#eee;color:#333;" onclick="closePmColSettings()">닫기</button>
          <button class="btn" style="flex:1;" onclick="applyPmColSettings()">적용</button>
        </div>
      </div>
    </div>
    ...
    <script>
      ...
      const PM_COLUMNS = [
        {{ key: 'branch', label: '지점', locked: false }},
        {{ key: 'item', label: '상품명', locked: true }},
        {{ key: 'code', label: '품번', locked: false }},
        {{ key: 'price', label: '구매금액', locked: false }},
        {{ key: 'supplier', label: '거래처', locked: false }},
        {{ key: 'tax', label: '과세설정', locked: false }},
      ...
      function getPmColSettings() {{
        try {{
          const saved = localStorage.getItem(PM_STORAGE_KEY);
          if (saved) return JSON.parse(saved);
        }} catch(e) {{}}
        const defaults = {{}};
        PM_COLUMNS.forEach(c => defaults[c.key] = true);
        return defaults;
      }}

      function applyPmColVisibility() {{
        const settings = getPmColSettings();
        PM_COLUMNS.forEach(c => {{
          const visible = settings[c.key] !== false;
          document.querySelectorAll('.pm-col-' + c.key).forEach(el => {{
            el.style.display = visible ? '' : 'none';
          }});
        }});
      }}

      function openPmColSettings() {{
        const settings = getPmColSettings();
        const container = document.getElementById('pmColCheckboxList');
        container.innerHTML = PM_COLUMNS.map(c =>
          '<label style="display:flex;align-items:center;gap:8px;' + (c.locked ? 'opacity:0.5;' : '') + '">' +
          '<input type="checkbox" class="pm-col-setting-check" data-key="' + c.key + '" style="width:18px;height:18px;flex-shrink:0;" ' +
          (settings[c.key] !== false ? 'checked' : '') + (c.locked ? ' disabled' : '') + '>' +
          '<span>' + c.label + (c.locked ? ' (필수)' : '') + '</span></label>'
        ).join('');
        document.getElementById('pmColSettingsModal').style.display = 'flex';
      }}

      function closePmColSettings() {{
        document.getElementById('pmColSettingsModal').style.display = 'none';
      }}

      function applyPmColSettings() {{
        const settings = {{}};
        document.querySelectorAll('.pm-col-setting-check').forEach(cb => {{
          settings[cb.dataset.key] = cb.checked;
        }});
        localStorage.setItem(PM_STORAGE_KEY, JSON.stringify(settings));
...
async def safety_stock_page(
    ...
    for r in existing_rows:
        existing_by_branch.setdefault(r["branch_name"], []).append(
            {"name": r["item_name"], "code": r["item_code"] or ""}
        )
    existing_by_branch_json = json.dumps(existing_by_branch, ensure_ascii=False)
    ...
    <div class="card">
      <h3 style="margin-bottom:8px;">새 안전재고 등록</h3>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <select id="ssBranch" style="flex:1;min-width:120px;" onchange="ssBranchChanged()">{branch_select_options}</select>
        <select id="ssItemSelect" style="flex:2;min-width:180px;" disabled onchange="ssItemSelectChanged()">
        ...
        <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#555;white-space:nowrap;">
          <input type="checkbox" id="ssManualMode" onchange="ssToggleManualMode()"> 새 상품 직접입력
        </label>
        <input type="text" id="ssItemNameManual" placeholder="상품명 직접입력" style="flex:2;min-width:160px;display:none;">
    ...
    <script>
      const EXISTING_BY_BRANCH = {existing_by_branch_json};
      ...
      function ssBranchChanged() {{
        ...
        const selectEl = document.getElementById('ssItemSelect');
        ssSelectedItem = null;
        if (!branch) {{
          selectEl.disabled = true;
          selectEl.innerHTML = '<option value="">지점을 먼저 선택하세요</option>';
          return;
        }}
        const items = EXISTING_BY_BRANCH[branch] || [];
        selectEl.disabled = false;
        if (items.length === 0) {{
          selectEl.innerHTML = '<option value="">이 지점에 등록된 상품 없음 (직접입력 사용)</option>';
        }} else {{
          selectEl.innerHTML = '<option value="">상품 선택</option>' +
            items.map((p, idx) => '<option value="' + idx + '">' + p.name + (p.code ? ' (' + p.code + ')' : '') + '</option>').join('');
        }}
      }}

      function ssItemSelectChanged() {{
        const branch = document.getElementById('ssBranch').value;
        const idx = document.getElementById('ssItemSelect').value;
        if (idx === '') {{ ssSelectedItem = null; return; }}
        const items = EXISTING_BY_BRANCH[branch] || [];
        ssSelectedItem = items[parseInt(idx)] || null;
      }}

      function ssToggleManualMode() {{
        const manual = document.getElementById('ssManualMode').checked;
        document.getElementById('ssItemSelect').style.display = manual ? 'none' : '';
        document.getElementById('ssItemNameManual').style.display = manual ? '' : 'none';
        ssSelectedItem = null;
      }}
      ...
      async function addSsItem() {{
        ...
        let itemCode = '';
        if (manual) {{
          itemName = document.getElementById('ssItemNameManual').value.trim();
        }} else if (ssSelectedItem) {{
          itemName = ssSelectedItem.name;
          itemCode = ssSelectedItem.code;
        }}

        if (!branch || !itemName || isNaN(qty)) {{
          alert('지점, 상품(선택 또는 직접입력), 안전재고수량을 입력하세요.');
          return;
        }}
        const res = await fetch('/master/purchase-order/safety-stock/save', {{
          method: 'POST', headers: {{ 'Content-Type': 'application/json' }},
          body: JSON.stringify({{ branch_name: branch, item_name: itemName, item_code: itemCode, qty: qty }})
        }});
        if (res.ok) {{ location.reload(); }} else {{
          const err = await res.json();
